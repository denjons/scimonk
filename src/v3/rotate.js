...
export function rotate(node,rad,origo){

  for(let i = 0; i < node.length/3; i++){
    rotatedNode[0] = node[i*3 + 0];
    rotatedNode[1] = node[i*3 + 1];
    rotatedNode[2] = node[i*3 + 2];
    // XZ rotation
    if(rad[0] !== 0){
    ...
    node[i*3 + 0] = rotatedNode[0];
    node[i*3 + 1] = rotatedNode[1];
    node[i*3 + 2] = rotatedNode[2];
  }
}
